...
static int ef_send_posix_error(file_descriptor *desc, ErlDrvTermData caller, int e)
{
    ErlDrvTermData spec[LOAD_ATOM_CNT + LOAD_INT_CNT
			+ LOAD_TUPLE_CNT];
    /* ErlDrvTermData caller = driver_caller(desc->port); /\* TODO: I doubt will this work? *\/ */
    int i = 0;
...
static void file_stop_select(ErlDrvEvent event, void* _)
{
/* TODO: should also work without sockets. we might need it for somethg else than sendfile()
   also, we are not supposed to close the socket here. if it happens I believe it's an
   error and that shouldn't happen at all as we don't own the socket at all */
/* #ifdef __WIN32__ */
/*     WSACloseEvent((HANDLE)event); */
/* #else */
/*     closesocket((int)event); */
/* #endif */
}
...
static void do_sendfile(file_descriptor *d)
{
    ...
    if (result_ok) {
	printf("===> DONE count=%ld eagain=%d\n",
	       (long int)count, d->sendfile.eagain);
